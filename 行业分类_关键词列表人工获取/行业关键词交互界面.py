...
class industry_classify(object):
    ...
    读取分词关键字对应行业分类的配置文件
    '''
    def __init__(self, yaml_path='industry.yaml'):
    ...
    def save(self):
        tmp_yaml_path = 'industry'+str(self.index)+'.yaml'
        self.index += 1
...
def run(startid=None, endid=None):
    ...
    with open('all_new.csv', mode='r', encoding='utf-8') as f, \
            open('fgood_all_new.csv', mode='w',encoding='utf-8') as fgood, \
            open('fbad_all_new.csv', mode='w', encoding='utf-8') as fbad:
        indexn = 1
        for iline in f:

            iline_list = iline.split(',')
            industry_tmp = iline_list[8]
...
def merge_yaml(file_path_list=['INDUSTRY_qq.yaml', 'industry_zy.yaml', 'industry_c.yaml']):
    ...
    tmp_yaml_path = 'industry' + "merge" + '.yaml'
    with open(tmp_yaml_path, mode='w', encoding='utf-8') as f:
